...
def conv1d_opt_x_int16_w_b1_notake(cnp.ndarray[DTYPE_X_t, ndim=2] x, cnp.ndarray[DTYPE_W_t, ndim=3] w, unsigned int dilation):
    ...
    with nogil:
        for ex in prange(num_examples, schedule='guided', num_threads=20):
            for h in range(H):
                for k in range(K):
